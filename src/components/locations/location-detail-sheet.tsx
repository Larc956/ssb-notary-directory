      ...
      <SheetContent className="overflow-y-auto sm:max-w-md w-full bg-slate-50 p-0">
        <div className="bg-white p-6 pb-4 border-b border-[#003A6C]/10 mt-4">
          <SheetHeader className="text-left space-y-1">
            <SheetTitle className="text-2xl font-black text-[#003A6C]">
              {location.display_name || location.name}
            </SheetTitle>
            {(location.raw_city || location.branch_name) && (
              <SheetDescription className="font-semibold text-[#003A6C]/60">
            ...
            <div className="flex flex-wrap gap-2 pt-3">
              ...
              {(location.student_discount_fee_text || location.student_discount_available) && (
                <Badge className="bg-[#F2A900] text-[#003A6C] hover:bg-[#F2A900]/90 font-bold border-none">
                  Student Discount
                </Badge>
              )}
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-3 text-sm text-[#003A6C]/80 bg-white p-4 rounded-xl border border-[#003A6C]/10 shadow-sm">
              {location.office_hours_text && (
                <div className="flex items-center gap-3"><Clock className="w-4 h-4 text-[#003A6C]/40" /><span className="font-medium">{location.office_hours_text}</span></div>
              )}
              {location.contact_numbers_text && (
                <div className="flex items-center gap-3"><Phone className="w-4 h-4 text-[#003A6C]/40" /><span className="font-medium">{location.contact_numbers_text}</span></div>
              )}
              {location.email && (
                <div className="flex items-center gap-3"><Mail className="w-4 h-4 text-[#003A6C]/40" /><span className="font-medium">{location.email}</span></div>
              )}
              {location.facebook_page && (
                <div className="flex items-center gap-3"><Globe className="w-4 h-4 text-[#003A6C]/40" />
                  <a href={location.facebook_page.startsWith('http') ? location.facebook_page : `https://${location.facebook_page}`} target="_blank" rel="noreferrer" className="text-blue-600 font-medium hover:underline truncate max-w-[250px]">
                    {location.facebook_page}
                  </a>
                </div>
              )}
          ...
          <section className="space-y-3">
            ...
            <div className="bg-white p-4 rounded-xl border border-[#003A6C]/10 shadow-sm space-y-4">
          ...
          {(location.feedback || location.verification_notes) && (
            <section className="space-y-3">
              ...
              <div className="space-y-3">
                {location.feedback && (
                  <div className="bg-[#003A6C]/5 p-4 rounded-xl border border-[#003A6C]/20">
